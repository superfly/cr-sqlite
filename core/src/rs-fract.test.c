...
static void testAsOrdered() {
  ...
  // test move after
  rc = sqlite3_exec(db, "UPDATE todo_fractindex SET after_id = 4 WHERE id = 3", 0, 0,
                    0);
  assert(rc == SQLITE_OK);
  rc = sqlite3_prepare_v2(db, "SELECT ordering FROM todo WHERE id = 3", -1,
                          &pStmt, 0);
  ...
  */
  ...
  // insert between / insert after (id=2 already inserted above; this is a
  // duplicate PK that silently fails — the ordering check below verifies the
  // original insert's result)
  rc = sqlite3_exec(db,
                    "INSERT INTO todo_fractindex (id, list_id, content, "
  ...
  */
  ...
  // move before
  // move 3 before 4
  rc = sqlite3_exec(db, "UPDATE todo_fractindex SET after_id = 2 WHERE id = 3", 0, 0,
                    0);
  assert(rc == SQLITE_OK);
  rc = sqlite3_prepare_v2(db, "SELECT ordering FROM todo WHERE id = 3", -1,
                          &pStmt, 0);
