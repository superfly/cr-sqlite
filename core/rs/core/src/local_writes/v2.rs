...
fn write_clock_entries(
    ...
    // For pk-only tables, use col_id=0 as sentinel. Otherwise, use col_ids
    // from the in-memory col_map (loaded at TableInfo creation).
...
pub fn v2_after_update(
    ...
    // Resolve non_pks indices to col_ids using the in-memory col_map.
    // After DROP COLUMN the map has holes, so the array index is not the packed col_id.
    ...
    for &col_idx in changed_col_indices {
        ...
        match tbl_info.col_map.iter().find(|(_, name)| name == col_name) {
            Some((col_id, _)) => col_ids.push(*col_id),
            None => {
                return Err(format!(
